...
数组的特点是易寻址，插入和删除困难;链表正好相反。HashMap 综合两者的特点，是一种易寻址，删除和插入也效率较高的数据结构。存储结构见下 Entry。
...
- initial capacity：桶的数量，table 数组的大小

- load factor：hashMap 容量自动增加前可以装多满

对于使用拉链法的 hashMap 来说，查找一个元素的平均时间是 O(1+n/2)`n 指的是链的长度`，是一个常数。**特别地，若负载因子越大，那么对空间的利用更充分，但查找效率的也就越低；若负载因子越小，那么哈希表的数据将越稀疏，对空间造成的浪费也就越严重**。【系统默认为 0.75，这是时间和空间成本上一种折衷】
...
利用 hash 算法快速存取元素。如果唯一性通过 equals 实现，随着元素增多，put 和 get 的效率将会越来越低。 

1. 通过 hasCode 快速找到某个桶
2. hasCode 可能存在碰撞的情况，通过 equals 逐个比较判断 key 是否存在
    - 存在，使用新 value 值替换旧 value 值
    - 不存在，存放至 Entry<K,V> 桶中
