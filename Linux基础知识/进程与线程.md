...
```
但是该资源不包括 CPU 时间，缺少 CPU 时间会从运行态转换为就绪态
```
...
进入区和退出区是负责实现进程互斥的代码。进程互斥的原则为：空闲让进、忙则等待、有限等待、让权等待`释放 cpu`。
...
信号量（Semaphore）是一个整型变量，可以对其执行 P`down` 和 V`up` 操作。
...
void P (semaphore S) {
    S--
    if (S.value < 0) {
        block(S.Q) // 阻塞当前进程(原语)，并 push 到等待队列中
    }
}
void V (semaphore S) {
    S++
    if (S.value <= 0) {
        wakeup(S.Q) // 唤醒进程(阻塞态 -> 就绪态)
    }
}
```
...
管道就是内存中开辟的一个固定的缓冲区。连接读写进程的一个共享文件。
...
- 只支持**半双工通信**（单向**互斥传输【buffer写满才可以读数据，buffer读空才可以写数据】**）
